// Generated by using Rcpp::compileAttributes() -> do not edit by hand
// Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

#include <Rcpp.h>

using namespace Rcpp;

#ifdef RCPP_USE_GLOBAL_ROSTREAM
Rcpp::Rostream<true>&  Rcpp::Rcout = Rcpp::Rcpp_cout_get();
Rcpp::Rostream<false>& Rcpp::Rcerr = Rcpp::Rcpp_cerr_get();
#endif

// interpolate_pitchpoints
NumericVector interpolate_pitchpoints(const NumericVector& new_times, const NumericVector& old_times, const NumericVector& pitch_vals);
RcppExport SEXP _sosprosody_interpolate_pitchpoints(SEXP new_timesSEXP, SEXP old_timesSEXP, SEXP pitch_valsSEXP) {
BEGIN_RCPP
    Rcpp::RObject rcpp_result_gen;
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< const NumericVector& >::type new_times(new_timesSEXP);
    Rcpp::traits::input_parameter< const NumericVector& >::type old_times(old_timesSEXP);
    Rcpp::traits::input_parameter< const NumericVector& >::type pitch_vals(pitch_valsSEXP);
    rcpp_result_gen = Rcpp::wrap(interpolate_pitchpoints(new_times, old_times, pitch_vals));
    return rcpp_result_gen;
END_RCPP
}

static const R_CallMethodDef CallEntries[] = {
    {"_sosprosody_interpolate_pitchpoints", (DL_FUNC) &_sosprosody_interpolate_pitchpoints, 3},
    {NULL, NULL, 0}
};

RcppExport void R_init_sosprosody(DllInfo *dll) {
    R_registerRoutines(dll, NULL, CallEntries, NULL, NULL);
    R_useDynamicSymbols(dll, FALSE);
}
